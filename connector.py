    ...
    return [
        {
            ...
            "primary_key": ["stor_id", "ord_num"],  # Primary key column(s) for the table, optional.
...
def fetch_and_upsert(cursor, query, table_name: str, state: dict, batch_size: int = 1000):
    ...
    while True:
        ...
        for row in results:
            ...
            if row_data["date"] and row_data["date"].isoformat() > last_created:
                last_created = row_data["date"].isoformat()
